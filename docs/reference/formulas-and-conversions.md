...
$$
SV = 1402.392 + \Delta T + \Delta S + \Delta P + \Delta_{cross}
$$
...
| Medwin (simplified) | Quick estimates | Same form as Chen-Millero but uses depth \(D\) directly |
...
2. Remaining = \(0.567890 \times 60 = 34.0734\) minutes
3. Minutes = integer part = 34
4. Seconds = \(0.0734 \times 60 = 4.404\) seconds

**Result: 54° 34' 04.404"**

### DMS to Decimal Degrees

Given DMS = 54° 34' 04.404":

$$
DD = 54 + \frac{34}{60} + \frac{4.404}{3600} = 54.567890
$$

### Useful Distance Approximations

| At Latitude | 1° Latitude | 1° Longitude | 1' Latitude | 1' Longitude | 1" Latitude | 1" Longitude |
|:--|:--|:--|:--|:--|:--|:--|
| 0° (equator) | 110.574 km | 111.320 km | 1842.9 m | 1855.3 m | 30.715 m | 30.922 m |
| 30° | 110.852 km | 96.486 km | 1847.5 m | 1608.1 m | 30.792 m | 26.802 m |
| 60° | 111.412 km | 55.800 km | 1856.9 m | 930.0 m | 30.948 m | 15.500 m |
...
$$
\sigma_{total} = \sqrt{e_1^2 + e_2^2 + e_3^2 + \cdots + e_n^2}
$$
...
$$
TVU = \sqrt{0.25^2 + (0.0075 \times 30)^2} = \sqrt{0.0625 + 0.0506} = \sqrt{0.1131} = \mathbf{0.336\,m}
$$
...
Given slant range \(R\) and angle from vertical \(\theta\):

$$
\text{Ground range} = R \sin\theta
$$

$$
\text{Depth (below transducer)} = R \cos\theta
$$
...
$$
\text{Ground range} = \sqrt{R_{slant}^2 - h^2}
$$

### Target Height from SSS Shadow

$$
h_{target} = \frac{L_{shadow} \times h_{altitude}}{R_{shadow\_end} - R_{target}}
$$

### USBL Position Error

The horizontal position error from a USBL measurement at slant range \(R\) with angular accuracy \(\sigma_\theta\):

$$
\varepsilon_{horizontal} = R \sin(\sigma_\theta)
$$

For small angles: \(\varepsilon_{horizontal} \approx R \times \sigma_\theta\) (in radians)

Example: 1000 m slant range, 0.1° angle accuracy:

$$
\varepsilon = 1000 \times \sin(0.1°) = 1000 \times 0.001745 = \mathbf{1.75\,m}
$$

### Layback (Towed Systems)

$$
\text{Layback} = \sqrt{L_{cable}^2 - d_{towfish}^2}
$$
...
- \(\rho\) = seawater density (typically 1025 kg/m³)
- \(g\) = gravitational acceleration (9.81 m/s²)

**Practical version:**

$$
D\,(m) = \frac{P\,(dBar)}{1.006}
$$

(for seawater at mid-latitudes)

Or more precisely: \(D\,(m) = P\,(dBar) \times 0.993\) (approximate, latitude-dependent)
...
For two points \((\phi_1, \lambda_1)\) and \((\phi_2, \lambda_2)\) in decimal degrees:

$$
\Delta N = (\phi_2 - \phi_1) \times 111120
$$

$$
\Delta E = (\lambda_2 - \lambda_1) \times 111120 \times \cos(\bar{\phi})
$$
...
$$
\text{Bearing} = \text{atan2}(\Delta E,\, \Delta N)
$$
...
$$
\lambda_{CM} = (\text{Zone} \times 6) - 183
$$

### UTM Scale Factor

At the central meridian: \(k_0 = 0.9996\)

At the zone edges (3° from CM): \(k \approx 1.0004\) (varies with latitude)
...
$$
\gamma \approx (\lambda - \lambda_{CM}) \times \sin\phi
$$
...
Point at 60° N, 3° E. UTM Zone 31 (CM = 3° E).

\(\gamma = (3 - 3) \times \sin(60°) = \mathbf{0°}\) (point is on the central meridian).

Point at 60° N, 6° E (zone edge):

\(\gamma = (6 - 3) \times \sin(60°) = 3 \times 0.866 = \mathbf{2.6°}\)
