---
title: "Resources & Further Reading"
tags:
  - handbook
  - beginner
  - reference
---

# Resources & Further Reading

Now that you've made it through the guide, here's where to go next. The wiki itself is packed with detailed technical content, and the links below will point you to the right places.

## Reference Tables & Formulas

The wiki's Reference section has detailed tables and formulas you'll use on the job:

- [Common Formulas and Conversions](../reference/formulas-and-conversions.md)
- [Coordinate Systems and Datums](../reference/coordinate-systems-datums.md)
- [TPU and Error Budgets](../reference/tpu-error-budgets.md)
- [Acceptance Criteria Reference](../reference/acceptance-criteria-reference.md)

## Industry Standards

- [Industry Standards Reference](../reference/industry-standards-reference.md) - IMCA, IHO, and other regulatory bodies

## Calibration Procedures

When you're ready to learn the step-by-step procedures, the Calibration section covers everything:

- [All Calibration Procedures](../calibration/index.md) - USBL, MBES, INS/DVL, sound velocity, attitude, gyro, dimensional control, and geophysical calibrations

## Equipment Guides

- [All Equipment Guides](../equipment/index.md) - INS, MBES, heave/MRU, side scan sonar, sub-bottom profiler, pipe tracker, and video systems

## ROV Operations

- [All ROV Guides](../rov/index.md) - Metrology, SLAM, pipeline survey, as-built, and construction support

## Field Checklists

- [Mobilisation Checklist](../field-notes/mobilisation-checklist.md)
- [Demobilisation Checklist](../field-notes/demobilisation-checklist.md)

## Recommended External Resources

- [IMCA](https://www.imca-int.com/) - International Marine Contractors Association
- [IHO](https://iho.int/) - International Hydrographic Organization
- [FIG](https://www.fig.net/) - International Federation of Surveyors
- [The Hydrographic Society](https://www.hydrographicsociety.org/)

## Training & Certifications

- IMCA survey competence scheme
- Cat A / Cat B hydrographic surveyor certification (FIG/IHO)
- Manufacturer-specific training (Kongsberg, Sonardyne, iXblue/Exail)
- HSE certifications (BOSIET, HUET, offshore medical)
